					...
					<div class="table100 ver1 m-b-110">
							...
							<div class="table100-body js-pscroll">
								<table border=0>
									<tbody>					
										<% 
										'response.write "<br>398 Llego"
										if TotalArea = "SI" and sFab <> "" and sMar = "" and sRan = "" then 
											'response.write "<br>435 If1"
											sAre = "0"
											iAre = 1
											for iPro = 0 to  ubound(gProductosTotalNacional,2)
												...
												for iInd = 0 to  ubound(gIndicadores,2)
													response.write "<tr class='row100 body'>"
														...
														response.write "</td>"
														response.write "<td width=10% class='cell100 column10 text-right'>"
														response.write "</td>"
													response.write "</tr>"
												next
											next					
										...
										if TotalArea = "SI" then 
											'response.write "<br>520 If2"
											for iPro = 0 to  ubound(gProductosTotalNacional,2)
												'response.write "<br>486 Llego"
												response.write "<tr class='row100 body'>"
													'Area
													response.write "<td width=10% class='cell100 column1'>"
														response.write gProductosTotalNacional(1,iPro)
										...
										for iPro = 0 to  ubound(gProductos,2)
											...
											response.write "<tr class='row100 body'>"
												...
												response.write "<td width=10% class='cell100 column6 text-center'>"
													if sTam <> "" then
														resta = 2
														if sSeg = "" then resta = resta + 2
														if sRan = "" then resta = resta + 2
														'response.write "<br>775 Resta:" & resta
														response.write gProductos(11 - resta,iPro) 
													end if
												response.write "</td>"
											response.write "</tr>"
											response.write "<td colspan=4  class='cell100'>"
...
Sub CalcularIndicador
	...
	Select Case Indicador
		...
		Case 14 'VolActoCompra Calcular 
				sql = ""
				sql = sql & " SELECT "
				sql = sql & " Tamano, "
				sql = sql & " Cantidad "
		...
		Case 15 'ValActoCompra Calcular
				sql = ""
				sql = sql & " SELECT "
				sql = sql & " Cantidad, "
				sql = sql & " Precio_Producto, "
		...
		Case 16 'UnidActoCompra Calcular
				sql = ""
				sql = sql & " SELECT "
				sql = sql & " Cantidad "
				sql = sql & " FROM "
		...
		Case 28 'CuotaMercVol Calcular
			...
			sql = sql & " And Id_RangoTamano = 0"
			sql = sql & " And id_Semana in( " & idSemana & ")"
			'response.write "<br>2337 sql:=" & sql
			'response.end
			rsx1.Open sql ,conexion
			'response.write "<br>257 LLEGO" 
			'response.end
			if rsx1.eof then
				...
				sql = sql & " And id_Semana in( " & idSemana & ")"
				'response.write "<br>2378 sql:=" & sql
				'response.end
				rsx1.Open sql ,conexion
				'response.write "<br>257 LLEGO" 
				'response.end
		...
		Case 30 'CuotaMercUnid Calcular - Listo
			sql = ""
			sql = sql & " SELECT "
			sql = sql & " Cantidad "
			sql = sql & " FROM "
			...
			sql = sql & " And Id_RangoTamano = 0 "
			sql = sql & " And id_Semana in( " & idSemana & ")"
			'response.write "<br><br>2508 sql:=" & sql
			'response.end
			rsx1.Open sql ,conexion
			'response.write "<br>257 LLEGO" 
			'response.end
			if rsx1.eof then
				...
				sql = sql & " And id_Semana in( " & idSemana & ")"
				'response.write "<br>2548 sql:=" & sql
				'response.end
				rsx1.Open sql ,conexion
				'response.write "<br>257 LLEGO" 
				'response.end
			...
			'response.write "<br><br>2572 TotalUnidades:" & TotalUnidades
			'response.write "<br>2573 TotalFiltro:" & TotalFiltro
