...
function obtener_DetallexProducto(id) {
	...
	if (idConsumo==="0" || idConsumo==null || idConsumo==undefined){
		...
		//
		// Ajustar y Validar Consumos Mercado Principal
		//
		...
		if (idTipConsumo === "1" || idTipConsumo === "8") {
			...
			.done(function(data) {
				...
				var idmoneda = parseInt(data[0].idmoneda);
				var idcateg  = parseInt(data[0].idcateg);
				var unidades = parseInt(data[0].unidademp);				
				//	
	  ...
	  swal(
		...
		function () {
			...
			if($("#tipocat").val()==5){
				//Yemas
				let strNum;
				valunidades = Math.abs(Unidades); /* Valor absoluto del número */				
				if (valunidades < 10){
					strNum= "0".repeat(2 - valunidades.toString().length) + valunidades.toString();					
				}else{
					strNum = valunidades.toString();
				}
				//
				valBarcode ="124643214797" + strNum;				
				//	
			}else{
				valBarcode = $("#txtCodigoBar").val();				
			}
