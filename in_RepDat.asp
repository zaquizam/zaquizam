...
.rr_dat 			{font-family:Verdana;font-weight:normal;height:20;font-size: 8pt;text-align: right;color: #000000;text-decoration: none;text-indent:8}
...
Sub rr_LeePar
	...
	'response.write "<br>Leepar rr_MesDes:= " & rr_MesDes
	'response.write "<br>Leepar rr_MesHas:= " & rr_MesHas
...
<table border="0" cellspacing="1" cellpadding="0" width="328" bgcolor="#cococo" align="right"  id="Table3">
    <%
	for iii=1 to rr_nCom 
	
	%>
    <tr>
        <td width="128px" valign="middle" bgcolor="#ffffff" align="right" height="20" background="/images/bcc2.gif">
        ...
        <td width="200px" valign= "middle" bgcolor="#ffffff" align="left" height="20">
 ...
 %>    
    <% if rr_ipexe=0 then
            if rr_iCombo<>1 then
     		...
     		%>
                <form action="<%=sPar %>" method='post' id="Form5">
...
Sub rr_VerTit
        ...
        <table border="<%=iBor%>" width="<%=iAnc %>" cellpadding="0" cellspacing="1" bgcolor="#c0c0c0" align="center" id="Table4">
            ...
            <tr  bgcolor="#ffffff">
                ...
                if rr_ipDis=1 And rr_ipExe=0 then  irp=irp+iColMas%>
...
' Construir el WHERE
	    ...
	    if rr_sPar(iCC,0)<>"Todas" then 
	        if rr_sPar(iCC,0)<>"" then 
	            select case rr_sCom(icc,4)
	                case 0 
                    sql= sql & "((" & rr_sCom(icc,10) & ")='" & rr_sPar(iCC,0) & "') AND "
                 case 1
                    sql= sql & "((" & rr_sCom(icc,10) & ") like'%" & rr_sPar(iCC,0) & "%') AND "
	...
	sql =mid(sql,1, ix-4)
	sqlW = sql & ") "
'response.write "<br><br>" & sqls
'response.write "<br><br>" & sqlw
...
' Guardar en rr_gData
  ...
  'response.write "<br>902 0:=" & gTem(0, ii) & " 1:="  & gTem(1, ii) & " 2:="  & gTem(2, ii)& " 3:="  & gTem(3, ii) & " ipp:=" & Ipp
    ...
    For iiReg=iiR to rr_iMaxReg 
        ...
        if sn1<>rr_gTit(iiReg,1) then
            ...
            if rr_ipGra=1 and rr_ipdis=0 then 
                ...
                <table width="950"  border="0" align="center"  cellspacing="0"  id="Table5">
                     <tr><td width="100%" height="350" valign="top" bgcolor="<%=sx%>">
                     <% 'd_GraLin rr_gData, rr_gTit,iiReg  
                      d_GraLinDat rr_gData, rr_gTit,iiReg  , 1
                     %>
                      </td></tr>    
                </table>       
            <%end if 
...
' Escribir Titulo del registro
                    ...
                    if rr_gTit(iiReg,0)=12 then%>
                       <td  class="rr_dim1" rowspan="<%=rr_nVar %>"  colspan="1" valign="middle"  align="center" >
                            <%=rr_gTit(iiReg,1 ) %>
                       </td>
                       <td  class="rr_dim1" rowspan="<%=rr_nVar %>"  colspan="2" valign="middle"  align="center" >
                            <%=rr_gTit(iiReg,2 ) %>
                       </td>
                        ...
                        <%isw=1
                          ...
                          for ixz= 1 to rr_nFil
                                if iiReg=0 and ixz=1 then
                                 ...
                                 %>
                            <td  onmouseover="this.style.backgroundColor='#eeeeee' "onmouseout="this.style.backgroundColor='#ffffff'" class="rr_dim1" rowspan="<%=rr_nVar%>" valign="middle"  align="center" >
                                    <%  'sp=Spro
							...
							if rr_gData(iiReg,iiCol, iiVar) then
								if rr_gData(iiReg,iiCol, iiVar)=0  then 
								    ...
								    elseif rr_sVar(iiVar,5)=2 then
									    iV=rr_gData(iiReg,iiCol, iiVar+1)/rr_gData(iiReg,iiCol, iiVar+2)*100
								        iV= iV *  rr_sVar(iiVar,1 )
								    else
								        ' Promedio
    								    if rr_sVar(iiVar,4)= 3 then
    								        if isTot=1 then rr_gData(iiReg,iiCol, iiVar)=rr_gData(iiReg,iiCol, iiVar)/rr_gData(iireg,iiCol, 10)
									    end if
								...
								'sVariable = rr_sVar(iiVar,0 )
								'if sVariable = "Variación" then
								'	if rr_gData(iiReg,iiCol, iiVar) = 0 then
								'		response.Write " 0"
								'	else
								'		response.Write " -"
...
Sub rr_VerVis
    ...
    sx=rr_sColFon
        ...
        %>
        <table border="0" width="<%=Iw%>" bgcolor="<%=sx %>" cellspacing="0" align="right">
            <% 
                'if rr_iCombo<>1 then
                if rr_iwGra=1 then
                ig = rr_ipGra
    ...
    sx=rr_sColFon
        ...
        <table border="0" width="<%=Iw%>%" bgcolor="<%=sx %>" cellspacing="0" align="right">
            ...
            <tr>
                ...
                <td width="33%">
                <form action="<%=sPar %>" method="post"  id="Form3">
                    <input type="submit" name="but1" value="<%=sx%>" id="Submit1" class="rr_but"/>
...
' Ver Fotos
            ...
            %>
                ...
                <td width="33%">
                        ...
                        <form action="<%=sPar %>" method="post"  id="Form2">
                        <input type="submit" name="but1" value="<%=sx%>" id="Submit3" class="rr_but"/>
            ...
            <td width="33%">
            <form action="<%=sPar %>" method='POST' id="Form1">
                <input type="submit" name="but1" value="Exportar a Excel" id="Submit5" class="rr_but"/>
            </td>
            </form>
            </tr>
...
Sub rr_Main 
    rr_ParDat
	'response.write "<br>1607 spro: " & spro
	if sPro <> "sp_rGeneralN.asp" then
		if isNull(rr_MesHas) or rr_MesHas = "" then
			Dim RsBus
			...
			rr_MesHas = RsBus.fields("Periodo_Pub_Mens_Hasta")
			rr_Mesdes = rr_MesHas - 4
			RsBus.close
		end if
    end if
	rr_Dimensiones
'response.write "<br> Time11:=" & Time:response.flush		
    ...
    <table width="950"  border="0" align="center" cellspacing="0" bgcolor="<%=rr_sColFon%>"  ID="Table2">
    <tr>
        <td>
  ...
  <td width="70%" align="center"><%=rr_sTit %>
    ...
    %>
    <div style="background-color:#ffffff; width: 950;float:left;padding: 0px 0px 20px 0px" >
        <%if rr_ipExe=0 then %>
        <div style="width:500px;background-color:#ffffff; float:left;vertical-align:middle;padding:3px 0px 3px 0px;"   title="" >
            <% rr_VerMeses %>
		</div>
		<%end if %>
        <div style="width:450px;background-color:#ffffff;float:left;vertical-align:middle;"  title="" >
             <% rr_VerCombo%>
        </div>   
	</div>
    <% 
       ...
       if rr_ilof = 1	 then %>
            ...
            <%sx=rr_sColFon
            if rr_ipExe=1 then sx="#ffffff"%>		    
		    <div style="background-color:<%=sx%>; width:950">
		        <br />
		        <% sBg="#cccccc"
...
Const RutaFoto= "http://retail.homeftp.net/Venezuela/campo/fotos/"

Sub rr_VerFotos(SqlFot,iFilas,iColumnas,iWidth,iHeight,sBg) 
	...
	rs.CursorType 		= 1 		' CursorType = Forward-Only
	rs.LockType 		= 1			'LockType = Read-Only
	rs.CursorLocation 	= 1	'CursorLocation = adUseClient
    'set rs = CreateObject("ADODB.Recordset")
'response.write "<br>1476 " & sqlfot
    rs.Open SqlFot,conexion

 	if rs.eof then
		response.write (" <center> No se encontraron Fotos ")		
		response.write("</center> ")
		exit sub
	End if		

' Obtener los datos con GetRows
	gDatosFotos = rs.GetRows
	rs.Close
	set rs = nothing

    iRegsPorPag = (iColumnas * iFilas)
    ixPag = rr_ipPag
	...
	if iFin > UBound(gDatosFotos, 2) then
		iFin = UBound(gDatosFotos, 2)
	end if
%>
	<br>

	<table border="0" width="98%" cellpadding="1" cellspacing="1" bgcolor="<%=sBg%>" align="center">
	    <tr bgcolor="<%=rr_sColFon%>"  >
	    <%  ix=0
	        ia=0
	        for i = icomienzo to ifin %>
	            <td align="center" bgcolor="<%=rr_sColFon%>" height="<%=iHeight+50%>" class="l2" >
                    <a href="/rtmaster/lib/ut_visfot.asp?fot=Venezuela/campo/fotos/<%=gDatosFotos(0,I)%>&zoo=560" target='Fotos' >
		            <img src="<%=RutaFoto & gDatosFotos(0,I)%>"   border="0" alt= '<%=gDatosFotos(2,I)& " - " & gDatosFotos(1,I)%>' height="<%=iHeight%>" width= "<%=iWidth%>" align="center" >
		           <br/>
		            <%	             
		            response.write(gDatosFotos(2,I)& "<br>"  )
		            response.write(gDatosFotos(1,I))%>
		            </a>
		        </td>
		        <%ia= ia +1
		        if ia= iColumnas then%> 
			        </tr>
	        ...
	        next
	        ix =ifin-icomienzo
	        if ix<icolumnas then ia=0
	        if ia<> 0 then 
	        if iFin+1>iColumnas then
	            ix=iColumnas-ia
	            response.write("    <td  align=center bgcolor=rr_sColFoncolspan=" & ix & " >")
		       ' response.write ("<font face= Tahoma size=2 color=#eeeccc><b>&nbsp;" & icomienzo & "-" & iFin & "-" & UBound(gDatosFotos, 2) & "-" & ia & "-" & icolumnas)
		        response.write(		"</td>")
...
' Ver Pagina de las fotos%>
    <br/ >
	<table border="0" cellspacing="0" cellpadding="0"  width="98%" bgcolor="<%=rr_sColFon%>" height="20" align="center">
	<tr>
	<td colspan=5  valign="middle"  bgcolor="<%=rr_sColFon%>" align="center" height="20">
	<hr noshade color="<%=sBg%>" size=1 width=100% align=center >
	</td>
	</tr>
	<%
	...
	Response.Write("<TD WIDTH=20% VALIGN=MIDDLE  BGCOLOR= " & rr_sColFon& " align=right height=20>")
	if ixPag > 1 then
		...
		response.write (sPar)
		response.write(">")%>
		<img src="\rtmaster\images\wztop.bmp" border="0"  style="text-decoration:none"></a>
	<%	
	end if
	Response.Write("</TD>")	
	
	Response.Write("<TD WIDTH=15% VALIGN=MIDDLE  BGCOLOR=#fffff0 align=center height=20>")
	if ixPag > 1 then
		ipP=rr_ipPag
		rr_ipPag=ixPag-1
		rr_calpar
		...
		response.write(">")
		%>
	    <img src="\rtmaster\images\wzback.bmp" border="0"  style="text-decoration:none"></a>	
	   <% 
	end if
	Response.Write("</TD>")


	Response.Write("<TD WIDTH=30% VALIGN=MIDDLE  BGCOLOR=#fffff0 align=center height=20>")
		response.write ("<font face= Verdana size=2 color=#800000><b>")
		Response.Write("Página " & ixPag & " de " & iPaginas & "</b>")
	Response.Write("</TD>")
	...
	if ixPag < iPaginas then
		...
		response.write(">")
		%>
        <img src="\rtmaster\images\wznext.bmp" border="0"  style="text-decoration:none"></a>
	<%end if
	Response.Write("</TD>")
	
	
	Response.Write("<TD WIDTH=20% VALIGN=MIDDLE  BGCOLOR=#fffff0 align=left height=20>")
	if ixPag < iPaginas then
	    ipP=rr_ipPag
		rr_ipPag=iPaginas
		rr_calpar
		...
		response.write (sPar & ">")
		%>
		<img src="\rtmaster\images\wzend.bmp" border="0"  style="text-decoration:none"></a>			
	<%
	end if
	response.write("</tr>")
	
	Response.Write("<TR >")		
	Response.Write("<TD colspan=5 VALIGN=MIDDLE  BGCOLOR=#fffff0 align=center height=20>")
	response.write("<hr noshade color=" & sBg & "  size=1 width=100% align=center >")
	Response.Write("</TD>")
	response.write("</tr>")
	Response.Write("<TR >")		
	Response.Write("<TD colspan=5 VALIGN=MIDDLE  BGCOLOR=#fffff0 align=center height=20>")
	response.write ("<font face= Verdana size=2 color=#800000>")
	response.Write("<center>Número de Fotos:= " & Ubound(gDatosFotos,2)+1)
...
Sub rr_VerMeses
    ...
    CalPar
    Dim RsBus
	'response.write "<br>1914 sPro:=" & sPro
	isw=0
	if sPro= "pr_vReportePricing.asp" then
		isw=1
	end if
    set RsBus = CreateObject("ADODB.Recordset")
    RsBus.CursorType = 1
    ...
    sql = ""
	sql = sql & " SELECT "
	sql = sql & " Min(pr_T_PeriodoReporte.IdConsecutivo) AS MínDeIdConsecutivo, "
	sql = sql & " pr_T_Periodo.Periodo "
	sql = sql & " FROM pr_T_PeriodoReporte INNER JOIN pr_T_Periodo ON pr_T_PeriodoReporte.Id_Periodo = pr_T_Periodo.IdPeriodo "
	if isw=1 then
		sql = sql & " Where "
		sql = sql & " pr_T_PeriodoReporte.IdConsecutivo > 41 "
	end if
	sql = sql & " GROUP "
	sql = sql & " BY pr_T_Periodo.Periodo, "
	sql = sql & " pr_T_Periodo.IdPeriodo "
	sql = sql & " ORDER BY "
	sql = sql & " pr_T_Periodo.IdPeriodo "
	'response.write "<br>1929 sql:=" & sql
    RsBus.Open sql ,conexion
	gxBus=rsBus.getrows
    RsBus.close
    ...
    sql = ""
	sql = sql & " SELECT "
	sql = sql & " Max(pr_T_PeriodoReporte.IdConsecutivo) AS MínDeIdConsecutivo, "
	sql = sql & " pr_T_Periodo.Periodo "
	sql = sql & " FROM pr_T_PeriodoReporte INNER JOIN pr_T_Periodo ON pr_T_PeriodoReporte.Id_Periodo = pr_T_Periodo.IdPeriodo "
	if isw=1 then
		sql = sql & " Where "
	...
	'response.write "<br>1929 sql:=" & sql
    RsBus.Open sql ,conexion
	gxBus1=rsBus.getrows
    RsBus.close
	'response.write "<br>1972 rr_MesDes:= " & rr_MesDes
	'response.write "<br>1972 rr_MesHas:= " & rr_MesHas
	'exit sub
...
<table style="width:460px; height:50px; background:#ffffff; border: 1px solid #ccdbe4; " align="center"  border="0">
    ...
    <td align="center" class="rr_dim1">Desde: 
    ...
    <%
		for iReg=0 to ubound(gxBus,2)
                ipExe=0
                iM = rr_MesDes
                rr_MesDes = gxBus(0,iReg)
                rr_CalPar
                rr_MesDes = iM
				iSi=cint(rr_MesDes) - cint(gxBus(0,iReg))
                %>
                <option value="<%=sPar%>"
                <%if iSi=0 then response.write " selected " %>
                >
                <%
                response.write  gxBus(1,iReg) '& "=" & rr_MesDes & "=" & gxBus(0,iReg) 
                %>
  ...
  <td  align="center" class="rr_dim1">Hasta:
    
    
    <select size="1" name="HastaMes" id="Select3"  onchange="location.href=this.options[this.selectedIndex].value"  style="font-family:Verdana;font-weight:normal;height:20;font-size:8pt; vertical-align: middle;">
    
    <%
		for iReg=0 to ubound(gxBus1,2)
                ipExe=0
                iM = rr_MesHas
                rr_MesHas = gxBus1(0,iReg)
                rr_CalPar
                rr_MesHas = iM
				iSi=cint(rr_MesHas) - cint(gxBus1(0,iReg))
                %>
                <option value="<%=sPar%>"
                <%if iSi=0 then response.write " selected " %>
                >
                <%
                response.write  gxBus1(1,iReg) '& "=" & rr_MesHas & "=" & gxBus1(0,iReg) 
                %>
  ...
  <td  align="center" class="rr_dim1">Promedio:
