...
function buscarDetallexProductoFactura() {
	...
	/*Si la consulta Fallo*/
	.fail(function() {
		swal("Alerta..! (df)","Algo salio mal, Intente de Nuevo\nde continuar este Mensaje Reportelo", "error");
	},'HTML');
		...
		.done(function(data) {
			...
			if(data==="True"){				
				...
				if (!esUltimoElementoSeleccionado) {
					...
					// var index = $('#cboIdConsumo')[0].selectedIndex;     				
    				// $("#cboIdConsumo option:eq(${index})").remove();
				...
				// }
				//
				var len = $("#cboIdConsumo option").length;
				if(len===1) { 
					Reset();
				}else{
					$("#cboIdConsumo").change();
...
//
$("#next").click(function() {
    var isLastElementSelected = $('#cboIdConsumo > option:selected').index() == $('#cboIdConsumo > option').length -1;
...
$("#prev").click(function() {
    var isFirstElementSelected = $('#cboIdConsumo > option:selected').index() == 0;
