//
// FuncionesV1.js // 12ene21 - 14ene21
//
function Reset() {
	//		
	$("#txtSemana").val("");	
...
//
function buscarDetallesxConsumo(){
	//
	$(function() {
		buscarAltaHogar();
	});
	...
	$.ajax({		
		...
		success: function (data) {
			...
			if(data=="False"){
				...
				//$("#txtObservaInvestigar").html(observa);			
			}else{
				$("#txtMotivoInvestigar").val(data);				
				$("#txtMotivoInvestigacion").val(data);
				//$("#txtObservaInvestigar").html(observa);			
			}
			$("#loader").html("");
		},
...
function buscarDetalleConsumoxDia() {
	...
	$.ajax({		
		...
		success: function (data) {							
			...
			//
			$("#cboCanal").val(canal);
			$("#cboCadena").val(cadena);			
			$("#MonedaPagoFactura").val(moneda);
	...
	function () {
		...
		$.ajax({		
			...
			success: function (data) {				
				...
				$("#loader").html("");								
				if(data==="True"){
					var pos = document.getElementById("cboHogar").selectedIndex;
					// $("#Pais").val(data[0].Id_Pais).change();					
					// llenarCmbHogaresInvestigados();
					$("#responderInvestigacion").modal("hide");					
					swal("Aviso..!", "Respuesta Enviada...!", "success");
					
					var len = $("#cboHogar option").length;
					if (len <= 2) {
						$("#cboHogar").prop("selectedIndex", 1);
						$("#cboHogar").prop("disabled", true);						
					} else {
						$("#cboHogar").prop("selectedIndex", 1);
						$("#cboHogar").prop("disabled", false);
					}											
				}else{
