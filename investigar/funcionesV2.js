	...
	$.ajax({		
		...
		success: function (data) {
			...
			if(data=="False"){
				...
				$("#txtMotivoInvestigar").val(data[0].motivo);				
				$("#txtMotivoInvestigacion").val(data[0].motivo);
				$("#txtPregunta").val(data[0].observa);			
			}
			$("#loader").html("");
		},
	});		
...
function enviarRespuestaInvestigacion() {
	...
	function () {
		...
		.done(function(data) {
			if(data==="True"){
				//var pos = document.getElementById("cboIdConsumo").selectedIndex;
