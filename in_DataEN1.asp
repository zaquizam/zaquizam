...
'response.write "<br>851 i:= "& i & " " & ed_rs1.fields(i).name & "  " & ed_rs1.fields(i).type
...
<div style=" overflow:Auto; width:100%"> 
	<table border="0" width="100%" cellpadding="0" cellspacing="0"   align="center" id="Table8" style=" border:1px solid  #ffffff;  border-top-left-radius: 8px;border-top-right-radius:8px;" >
...
<!-- TItulo -->
  ...
  <div style=" overflow:Auto;   max-height:440px; height:440px"> 
...
'response.write "<br>1398 ed_iNumCam:= " & ed_iNumCam
...
' Esctibir el detalle			
        ...
        'response.Write "<br>1390 ed_iNumCam:=" & ed_iNumCam
		 ...
		 Else 
			...
			Select Case TipCam(j,1)
				...
				case else
					...
					if ed_rs1.fields(j).type="2" then sAli ="center"
					if ed_rs1.fields(j).type="135"  then sAli ="center"
					%>
					<td   align="<%=sAli%>"  title="<%=ed_sCampo(j,5)%>" style="margin-right:13px; margin-left:13px; padding:0 10px 0 10px">
					<%
						     ...
						     <% if isnull(gData(j,i)) then
						            ...
						            if Len(gData(j,i))<50 then %>
						                <% select case TipCam(j,5)%>
						                    <%case 3 ' Integer%>
						                        <%if isnumeric(gdata(j,i)) then%>
							                        <%=formatnumber(gData(j,i),0)%>
...
document.querySelector('#fileSelect').addEventListener('click', function(e) {
  ...
  fileInput.click(); // Or, use the native click() of the file input.
			...
			<%case 8 ' Fecha
			    ...
			    if ed_sCampo(i,8)<>"" then sx=sGra(i)
			    if iFor=1 then%> 
 			        <input  type="text"  name="<%=ed_rs1.Fields(i).name%>" id="ed_Fecha<%=ixfec%>" value="<%=gData(i,0)%>"  maxlength="10" size="12" class="ed_cam" <%=ed_sCampo(i,3)%> form="FormPag2" tabindex="<%=i %>"   />
 			    <%else %>    
	    	        <input  type="text"  name="<%=ed_rs1.Fields(i).name%>" id="ed_Fecha<%=ixfec%>" value="<%=sx%>"  maxlength="10" size="12" class="ed_cam" <%=ed_sCampo(i,3)%>   form="FormPag2" />
	    	    <% end if %>    
...
' Todos
        ...
        if ed_iFil=0 then%>
            <td  width="<%=iwid %>" class="ed_ms2" bgcolor="#ffffff" >
                <%= "- Sin Filtro -"%></td>
        <%else %> 
            <td  width="<%=iwid %>"  class="ed_ms1" bgcolor="#ffffff" >  
    ...
    for i=0 to ubound(gMenFil,2)
        ...
        ix=ed_iFil-gMenFil(0,i)
        if ix=0 then%>
            <td  width="<%=iwid %>"  class="ed_ms2" bgcolor="#ffffff"  title='<%=gMenFil(1,i) %>'>
            <%= "" & gMenFil(1,i) & "" %></td>
         <%else %> 
            <td  width="<%=iwid %>" class="ed_ms1" bgcolor="#ffffff"  title='<%=gMenFil(1,i) %>'>  
                <a href="<%=sPar %>"  >
                <%="" & gMenFil(1,i)  %></a></td>
         <%end if %>
          ...
          <tr >
           <% for i=0 to Ubound(ed_gMenu2,2)
                  ...
                  if ix <> 0 then %>
                    ...
                    <td class="ed_ms1" width="<%=iWid%>"    title = "<%=ed_gMenu2(6,i)%>">
                          <%
                           spxx=sPro
                          sPro= ed_gMenu2(5,i)
                 ...
                 <%else%>    
                     <%if ed_gMenu2(1,i)<>"#" then %>
                        <td class="ed_ms2" width="<%=iWid%>"   title = "<%=ed_gMenu2(6,i)%>" > 
                            <%=ed_gMenu2(1,i)%>
                        </td>
                     <%else %>  
                       <td class="ed_ms2" width="<%=iWid%>"   title = "<%=ed_gMenu2(6,i)%>" > 
...
Sub DataE
    ...
    if ed_iPas<>4 then 
        ...
        <center>
        <div style="width:98%"><%ed_MenPri %></div><%
    end if %>   
    
	<div style="width:97%; margin-left:auto; margin-right:auto"><%Ed_Main%></div>
