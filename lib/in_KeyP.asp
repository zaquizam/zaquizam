...
' Mostrar readonly
				...
				%>  
				<select size=""  name="<%=ed_rs1.Fields(i).name%>" id="Select3"  title="<%=ed_sCampo(i,5)%>"   tabindex="<%=i %>"  class="<%=ed_Formato(i,2)%>" form="FormPag2">
...
<%    	
	for i=1 to ed_iCombo
       ...
       %>
	    ...
	    <tr>
	        <td width="<%=ed_sCombo(i,5)%>" align="right" style=" font-family: Calibri; font-size:11px; padding: 0px 10px 0 0 ">
	        <%=ed_sCombo(i,0)  %>:
            </td>
	        <td width="<%=ed_sCombo(i,6)%>" style="font-size:10px; background-color:#ffffff; color:#000000;">
	        <%if ed_iPas<> 4 then %>
	    	    <select size="1" name="per" id="Select1"   onchange ="location.href=this.options[this.selectedIndex].value"  style="width:100%; font-size:10px; font-family:Tahoma; padding:3px 0 3px 0 ">
            <%  if ed_sCombo(i,2)<>"" then   
    		       sP=ed_sPar(i,0)
    		       ed_sPar(i,0)=ed_sCombo(i,2)
            ...
            %>
        			<option value="<%=sPar%>"  <% if ed_sPar(i,0) =ed_sCombo(i,2) then response.Write"selected" %> style="width:100%; font-size:10px; font-family:Tahoma; padding:5px 0 5px 0 " >
				    <%="[" & ed_sCombo(i,2) & "]"%>
			        </option>
