...
<%
	' g_ValBuscarDetallesxProductosxFactura.asp // 02ENE21  - 23feb21
	'
	Session.lcid = 1034
	...
	IF ( CInt(idTipoConsumo) = 1 or Cint(idTipoConsumo) = 8 ) then
		...
		'
		sql = sql & " SELECT"
		sql = sql & " PH_Consumo_Detalle_Productos.Id_Consumo,"						'0
		sql = sql & " PH_Consumo_Detalle_Productos.Id_Consumo_Detalle_Productos,"	'1
		sql = sql & " PH_Consumo_Detalle_Productos.Tipo_codigo_barras,"				'2
		sql = sql & " PH_Consumo_Detalle_Productos.Numero_codigo_barras,"			'3
		sql = sql & " PH_Consumo_Detalle_Productos.Cantidad,"						'4
		sql = sql & " PH_Consumo_Detalle_Productos.Precio_producto,"				'5
		sql = sql & " PH_Consumo_Detalle_Productos.Moneda,"							'6
		sql = sql & " PH_Consumo_Detalle_Productos.id_Moneda,"						'7
		sql = sql & " PH_Consumo_Detalle_Productos.Tasa_de_cambio,"					'8
		sql = sql & " PH_CB_Producto.Producto AS descripcion,"						'9
		sql = sql & " PH_CB_Producto.CodigoBarra,"									'10
		sql = sql & " PH_Consumo_Detalle_Productos.Validado,"						'11
		sql = sql & " PH_Consumo_Detalle_Productos.Pendiente,"						'12
		sql = sql & " PH_Consumo_Detalle_Productos.id_categoria"					'13		
		sql = sql & " FROM"
		sql = sql & " PH_Consumo_Detalle_Productos"
		sql = sql & " LEFT JOIN PH_CB_Producto ON PH_Consumo_Detalle_Productos.Numero_codigo_barras = PH_CB_Producto.CodigoBarra"
		sql = sql & " AND PH_CB_Producto.Ind_activo = 1"
		sql = sql & " WHERE"
		sql = sql & " PH_Consumo_Detalle_Productos.Id_Consumo = " & idConsumo
		'
		' Response.Write sql
...
<%
						...
						For i = 0 to ubound(arrDetalleProductos, 2)
							...
							' end if
							'
							if(arrDetalleProductos(8,i)=2) then
								'Bolivares
								Total = (CBDl(arrDetalleProductos(4,i)) * CDBl(arrDetalleProductos(5,i) ))								
							else
								Total = (CBDl(arrDetalleProductos(4,i)) * ( CDBl(arrDetalleProductos(5,i) ) * CDBl(arrDetalleProductos(8,i) ) ) )"
							end if
							'
							if(arrDetalleProductos(13,i)=9) then
								'Cheese
								Total = (CBDl(arrDetalleProductos(5,i)) )							
							end if
