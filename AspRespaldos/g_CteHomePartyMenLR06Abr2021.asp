	...
	Select Case Indicador
		...
		Case 9 'CompraMedHog
			...
			if rsx1.eof then
				...
				'31Mar2021-1
				'sql = sql & " Id_Consumo "
				'response.write "<br>1946 sql:=" & sql
				'response.end
				rsx1.Open sql ,conexion
				'response.write "<br>257 LLEGO" 
				'response.end
		...
		Case 28 'CuotaMercVol Calcular Revisando
			...
			sql = sql & " And Id_RangoTamano = 0"
			sql = sql & " And id_Semana in( " & idSemana & ")"
			'response.write "<br>2993 sql:=" & sql
			'response.end
			rsx1.Open sql ,conexion
			'response.write "<br>257 LLEGO" 
			'response.end
			if rsx1.eof then
				...
				sql = sql & " And id_Semana in( " & idSemana & ")"
				'response.write "<br>3038 sql:=" & sql
				'response.end
				rsx1.Open sql ,conexion
				'response.write "<br>257 LLEGO" 
				'response.end
		...
		Case 32 'PenetRelativa Calcular 
			sql = ""
			sql = sql & " SELECT "
			sql = sql & " Id_Hogar AS Total "
			sql = sql & " FROM "
		...
		Case 35 'HogRecomp (nuevos)
			if idSemana = "16,17,18,19" then cMes = 1
			if idSemana = "20,21,22,23" then cMes = 2
			...
			if cMes = 2 Then 
				...
				if rsx1.eof then
					...
					if rsx1.eof then
						...
						NoRepite = 0
						for iReg = 1 to 10000
							if vHogares37(iReg,2) = 1 then Total = Total  + 1
